...
float A[3][3] = {
    {1.173156, -0.013227, -0.007509},
    {-0.013227, 1.271835, -0.063959},
    {-0.007509, -0.063959, 1.272200}
};
float b[3] = {-16.882995, -5.798997, 35.620970};
...
int determine_orientation_area(float value, float range_min, float range_max, int num_sections, int hysteresis_margin, int prev_area) {
    float section_width = (range_max - range_min) / float(num_sections);

    if(prev_area != -1){

        if(prev_area == 0 && (value < range_max - hysteresis_margin && value > range_max - section_width)){
            return num_sections - 1;
        } else if (prev_area == num_sections - 1 && (value > range_min + hysteresis_margin && value < range_min + section_width)){
            return 0;
        }

        float lower_bound = range_min + prev_area * section_width;
        float upper_bound = lower_bound + section_width;

        if(value < lower_bound - hysteresis_margin && value > lower_bound - section_width){
            return max(prev_area - 1, 0);
        } else if (value >= upper_bound + hysteresis_margin && value < upper_bound + section_width){
            return min(prev_area + 1, num_sections - 1);
        } else {
            return prev_area;
        }
...
int determine_accel_area(float max_accel, float range_min, float range_max, int num_sections){
...
void update_area_window(int roll_area, int pitch_area, int yaw_area, int accel_area, int* window, int& index, int size) {
    if (index >= size * 4) {
        memmove(window, window + jump_size * 4, (size - jump_size) * 4 * sizeof(int));
        index -= jump_size * 4;
    }
    window[index++] = roll_area;
    window[index++] = pitch_area;
    window[index++] = yaw_area;
    window[index++] = accel_area;
}
...
void loop() {
    ...
    if (dt >= LoopTime) {
        ...
        int roll_area = determine_area(MadgwickResult.roll, roll_range[0], roll_range[1], num_sections, hysteresis_margin, prev_roll_area);
        int pitch_area = determine_area(MadgwickResult.pitch, pitch_range[0], pitch_range[1], num_sections, hysteresis_margin, prev_pitch_area);
        int yaw_area = determine_area(MadgwickResult.yaw, yaw_range[0], yaw_range[1], num_sections, hysteresis_margin, prev_yaw_area);
        if (data.accel_m > max_accel){
            max_accel = data.accel_m;
            // Serial.println(max_accel);
        }

        if (roll_area != prev_roll_area || pitch_area != prev_pitch_area || yaw_area != prev_yaw_area) {
            int accel_area = determine_accel_area(max_accel, accel_range[0], accel_range[1], num_sections);

            update_area_window(roll_area, pitch_area, yaw_area, accel_area, area_window, window_index, window_size);

            prev_roll_area = roll_area;
            prev_pitch_area = pitch_area;
            prev_yaw_area = yaw_area;
            max_accel = 0;
            float entropy = 0.;
            String highest_prediction;

            predict_gesture(area_window, window_index, window_size, entropy, highest_prediction);
